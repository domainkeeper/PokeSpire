...
import type { TypeEffectiveness, PokemonType } from './schemas/index.ts';
...
export type { PokemonSpeciesData, MoveData, AbilityData, ItemData, TypeEffectiveness, EvolutionData, PokemonType } from './schemas/index.ts';
`;
...
function writeManifest(data, spriteStats, cryStats) {
  const manifest = {
    ...
    sources: {
      pkmnDex: { version: '@pkmn/dex', speciesCount: data.species?.species?.length || 0, moveCount: data.moves?.moves?.length || 0, abilityCount: data.abilities?.abilities?.length || 0, itemCount: data.items?.items?.length || 0, typeCount: data.types?.types?.length || 0, evolutionCount: data.evolutions?.evolutions?.length || 0 },
